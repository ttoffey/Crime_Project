...
engine = create_engine(
    'postgresql://postgres:password@localhost:5432/crime')
...
def sunburst_map(query_year):
    ...
    tot_state_crime=pd.read_sql("""SELECT state_abbr,sum(cast(crime_reported as int)) as tot_state_crime
    FROM all_crime
    group by state_abbr
    ;""",engine)
    tot_year_df=pd.read_sql("""SELECT sum(cast(crime_reported as int)) as tot_year_crime
    FROM all_crime
    ;""",engine)
...
def choro_map(crime,newyear):
    ...
    with open('./App/new.geojson', 'r') as f:
        data = json.load(f)
