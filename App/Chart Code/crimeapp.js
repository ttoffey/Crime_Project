...
function choro_map(crime, newyear) {
  ...
  d3.json(`/choropeleth_map/${crime}/${newyear}`).then(function (data) {
    ...
    geojson = L.choropleth(data, {
      ...
      // Binding a popup to each layer
      onEachFeature: function (feature, layer) {
        layer.bindPopup(` Year: ${newyear}  ` + `Crime Type:  ` + feature.properties.density + `  State :` +
          feature.properties.name);
      }
    }).addTo(myMap);
    // Set up the legend.
  ...
  function createChart(state) {
    ...
    d3.json(`/state_data/${state}`).then((data) => {
      ...
      data.forEach((entry) => {
        ...
        popList.push(Object.values(entry)[4]/100);
      ...
      let layout = {
        title: 'STATE STATISTICS - Totals Per Year',
        xaxis: {
          tickangle: -45
        },
        yaxis: {
