function init() {
  ...
  //       // Binding a popup to each layer
  //       onEachFeature: function(feature, layer) {
  //         layer.bindPopup(` year ${newyear}` + `crime type : ${crime}` + feature.properties.density + `state :`+
  //         feature.properties.name);
  //       }
  //     }).addTo(myMap);
  //     // Set up the legend.
  ...
  function optionChanged(newyear) {
    ...
    function chartData(state_type) {
    d3.json("/dropdownstates").then(function (data) {
       
        let stateVal = state_type;
      ...
      function createChart(state) {
        ...
        d3.json(`/state_data/${state}`).then((data) => {
          ...
          let trace0 = {
            ...
            type: 'bar',
            name: 'Population Total (Millions)',
